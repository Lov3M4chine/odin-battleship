...
} = require("./highlightShipPlacementModule");
...
const computerAIModule = (function () {
  const processingModule = (function () {
    ...
    function processWithPreviousSunk(appContext) {
      ...
      appContext.attackCellData.randomCellNumber =
        randomizeCellSelected(appContext);
      appContext.attackCellData.cellShipName =
        appContext.playerOne.gameboardState.gameboard[
          appContext.attackCellData.randomCellNumber
        ].name;
      appContext.attackCellData.cellHTML = document.getElementById(
        `playerone-cell-${appContext.attackCellData.randomCellNumber}`
      );
    ...
    function processPreviousSunkData(appContext) {
      ...
      initializePlaceShipsModule.updateMessageBox(
        appContext,
        `It's your turn.`
      );
    }
    ...
    function processMiss(appContext) {
      if (appContext.attackCellData.calculatedCellNumberToAttack) {
        appContext.playerOne.gameboardState.gameboard[
          appContext.attackCellData.calculatedCellNumberToAttack
        ].isMiss = true;
        highlightBattleModeModule.highlightMiss(
          appContext.attackCellData.cellHTML
        );
      } else {
        appContext.playerOne.gameboardState.gameboard[
          appContext.attackCellData.randomCellNumber
    ...
    function updateIsVertical(appContext) {
      if (
        Math.abs(
          appContext.attackCellData.previousHitCoordinates[0] -
            appContext.attackCellData.previousHitCoordinates[1]
        ) === appContext.verticalSize
      ) {
  ...
  const validateCellsModule = (function () {
    ...
    function wasCellPreviouslySelected(appContext) {
      if (
        appContext.attackCellData.isCellHit ||
        appContext.attackCellData.isCellMiss
      ) {
        return true;
      }
    }
    ...
    function validateAttackCell(appContext) {
      ...
      appContext.cellSelected =
        appContext.attackCellData.calculatedCellNumberToAttack;
      if (
        highlightShipPlacementModule.checkPlacementModule.isCellOutOfBounds(
          appContext.attackCellData.calculatedCellNumberToAttack,
          appContext
        ) ||
        isCellBeyondHoriztonalSize(appContext) ||
        wasCellPreviouslySelected(appContext)
      ) {
        appContext.attackCellData.validateAttackCellCollection.push(
          appContext.attackCellData.calculatedCellNumberToAttack
        );
        randomizeAttackCell(appContext);
      } else {
...
function calculateRandomPossibleCells(appContext) {
  let smallestCoordinate = Math.min(
    ...appContext.attackCellData.previousHitCoordinates
  );
  let largestCoordinate = Math.max(
    ...appContext.attackCellData.previousHitCoordinates
  );
  ...
  if (appContext.orientation.isVertical === true) {
    randomPossibleCellNumberSmall =
      smallestCoordinate - appContext.verticalSize;
...
function randomizeAttackCell(appContext) {
  ...
  if (calculatedCells.randomizeTheFourNumbers) {
    ...
    let calculatedCellNumber =
      appContext.attackCellData.calculatedCellNumberToAttack;
    if (
      calculatedCellNumber >= 0 &&
      calculatedCellNumber <
        appContext.horizontalSize * appContext.verticalSize &&
      !compareValidationCollection(appContext, numbers)
    ) {
      appContext.attackCellData.isCellHit =
        appContext.playerOne.gameboardState.gameboard[
          calculatedCellNumber
      ...
      appContext.attackCellData.cellHTML = document.getElementById(
        `playerone-cell-${calculatedCellNumber}`
      );
    ...
    if (randomValue < 0.5) {
      appContext.attackCellData.calculatedCellNumberToAttack =
        calculatedCells.randomPossibleCellNumberSmall;
      let calculatedCellNumber =
        appContext.attackCellData.calculatedCellNumberToAttack;
      if (
        calculatedCellNumber >= 0 &&
        ...
        appContext.attackCellData.cellHTML = document.getElementById(
          `playerone-cell-${appContext.attackCellData.calculatedCellNumberToAttack}`
        );
        computerAIModule.validateCellsModule.validateAttackCell(appContext);
      } else {
        appContext.attackCellData.validateAttackCellCollection.push(
      ...
      appContext.attackCellData.calculatedCellNumberToAttack =
        calculatedCells.randomPossibleCellNumberLarge;
      let calculatedCellNumber =
        appContext.attackCellData.calculatedCellNumberToAttack;
      if (
        calculatedCellNumber >= 0 &&
